public class Part1 {
    public static void main(String[] args) {
        ...
        try (BufferedReader bufferedReader = new BufferedReader(new FileReader(cheminFichier))) {
            ...
            for (Map.Entry<String, Integer> entry : zero.entrySet()) {
                ...
                HashSet<String> neuf = new HashSet<>();
                Parcourir(carte, x, y, neuf, 0);
                result += neuf.size();
    ...
    public static int Parcourir(char[][] carte, int x, int y, HashSet<String> neuf, int last) {
        int[] dx = {-1, 1, 0, 0};
        int[] dy = {0, 0, -1, 1};
        ...
        for (int i = 0; i < 4; i++) {
            if (x + dx[i] >= 0 && x + dx[i] < carte.length && y + dy[i] >= 0 && y + dy[i] < carte[0].length) {
                ...
                if (carte[x + dx[i]][y + dy[i]] == '9' && carte[x][y] == '8') {
                    neuf.add((x+dx[i]) + ", " + (y+dy[i]));
                }

                else if (carte[x + dx[i]][y + dy[i]] == lasts) {
                    Parcourir(carte, x + dx[i], y + dy[i], neuf, carte[x + dx[i]][y + dy[i]]-'0');
                }
